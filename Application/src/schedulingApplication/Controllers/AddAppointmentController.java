    ...
    public void onSaveButtonPress(ActionEvent event) throws InvalidDataException, SQLException, IOException{
        ...
        if(customersChoiceBox.getSelectionModel().getSelectedItem() == null){
            ...
            for(int i = 0; i < LocalData.getAppointments().size(); i++){
                ...
                }else if((newAppointmentStartTime < startOfBizHours || newAppointmentStartTime > endOfBizHours) || newAppointmentEndTime > endOfBizHours || newAppointmentStartTime < startOfBizHours){
                    outOfBizHours = true;
                    try{
            ...
            if(!overlapping && !outOfBizHours){
                try{
                    ...
                    Database.addAppointment(title,customer,description,location,contact,url,day,adjustTime(startHour),startMinute,adjustTime(endHour),endMinute);
                    goToAppointments(event);
